	...
	for (const message of conversation.messages ?? []) {
		for (const media of message.media ?? []) {
			...
			if (!conversation.media)
				debug('Missing media object in conversation with messages with media', $state.snapshot(conversation));
		...
		for (const mediaID of message.mediaIDs ?? []) {
			if (!conversation.media?.find((media) => media.id === mediaID)) {
				debug(
					'Media ID not found in conversation media',
					$state.snapshot(mediaID),
					$state.snapshot(conversation.media)
				);
			}
		}
	}
