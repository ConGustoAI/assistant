...
<div class="relative h-full w-full">
	{#if disabled}
		<div
			class={cn(
				'field h-fit min-h-10 w-full resize-none overflow-auto px-3 py-2 text-base whitespace-pre-wrap',
				className
			)}>
			{value}
		</div>
	{:else}
		...
		<div
			...
			{onkeydown}
			{onchange}
			class={cn('field h-full min-h-10 overflow-y-auto px-3 py-2 text-base whitespace-pre-wrap', className)}>
		</div>
		{#if value === '' && !focused}
